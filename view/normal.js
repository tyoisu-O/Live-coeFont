...
function time(){
    ...
    if (nowMin % 4 === 0) {
        ...
        if (nowSec === 1 && nowMin % 4 === 1) {
            $('#weather_flame_div').css('opacity', 1)
        }
        if (nowSec === 58 && nowMin % 4 === 3) {
            $('#weather_flame_div').css('opacity', 0)
        }
    }

    // bgmフェード
    if (nowSec <= 5 && nowMin % 4 === 0) {
        bgm2.volume += 0.02;
    }
    if (nowSec >= 55 && nowMin % 4 === 0) {
        bgm2.volume -= 0.02;
    }

    if (nowSec <= 10 && nowMin % 4 === 1) {
        bgm.volume += 0.01;
    }
    if (nowSec >= 50 && nowMin % 4 === 3) {
        bgm.volume -= 0.01;
    }
